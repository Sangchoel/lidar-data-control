...
#include <algorithm> // std::sort를 사용하기 위함
#include <vector>
#include <cmath>
...
public:
    ...
    std::vector<std::pair<double, double>> processScan(const sensor_msgs::LaserScan::ConstPtr& scan) {
        ...
        std::vector<float> filtered_ranges = applyMedianFilter(scan->ranges, 5); // 중간값 필터 적용
    ...
    std::vector<float> applyMedianFilter(const std::vector<float>& input, int window_size) {
        ...
        for (size_t i = 0; i < input.size(); ++i) {
            for (int j = -half_window; j <= half_window; ++j) {
                ...
                window[j + half_window] = input[index];
            }
            std::nth_element(window.begin(), window.begin() + half_window, window.end());
